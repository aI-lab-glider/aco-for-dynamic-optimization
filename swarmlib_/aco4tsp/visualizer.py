...
class Visualizer(VisualizerBase):  # pylint:disable=too-many-instance-attributes
    ...
    def replay(self, **kwargs):
        ...
        def _update(num):
            ...
            #     f'{graph.name} - Iteration ({num+1}/{len(self.__edge_colors)})')

            nodes_artist = nx.draw_networkx_nodes(
                graph.networkx_graph, pos=node_pos, ax=ax, node_color=self.__node_color)
            labels_artist = nx.draw_networkx_labels(
                graph.networkx_graph, pos=node_pos, ax=ax)
            edges_artist = nx.draw_networkx_edges(
                graph.networkx_graph, pos=node_pos, width=self.__edge_widths[num], edge_color=self.__edge_colors[num], ax=ax)

            return nodes_artist, labels_artist, edges_artist

        a = animation.FuncAnimation(fig, _update, frames=len(
            self.__edge_colors), interval=self.__interval, repeat=self.__continuous)
        a._start()
        plt.show()
