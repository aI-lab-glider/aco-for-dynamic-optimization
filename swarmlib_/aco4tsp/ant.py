...
class Ant(Thread):
    def __init__(self, start_node, graph: 'Graph', alpha, beta, Q, use_2_opt, random, c):
    ...
    def run(self):
        ...
        while possible_locations:
            ...
            if self._load + demand <= self._capacity:
                ...
                self.__move_to_next_node((a, self.traveled_nodes[0]))
                self._load = 0
        ...
        # Move back to origin
        if self.__current_node != self.traveled_nodes[0]:
            self.__move_to_next_node(
                (self.__current_node, self.traveled_nodes[0]))
